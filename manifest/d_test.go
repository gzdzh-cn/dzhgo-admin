...
func doExcel2() {
	...
	e := excel.NewExcel()
	e.SetExcelSheetData(mapData).SaveExcel("./")
...
func doExcel() {
	...
	e := excel.NewExcel()
	e.SetSheet("Sheet1").SetCellHead(headList).SetCellRow(dataList)
	e.SetSheet("Sheet2").SetCellHead(headList2).SetCellRow(dataList2)
	e.SaveExcel("./")
}
...
func getFileSize() {
	...
	logger.Debugf(ctx, "get file size:%v", gfile.Size("./data/logs/2.log"))
}
...
func clearData(ctx context.Context, tx gdb.TX, tables string) error {
	...
	for _, table := range tableSlice {
		...
		logger.Warningf(ctx, "清空：%v", table)
	}
...
func insertDataByQueue(tx gdb.TX, table string, insertData interface{}, isUnscoped bool) error {
	...
	logger.Warningf(ctx, "写入数据：%v", table)
	return nil
}
...
func TestJoinDaoSql(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		...
		logger.Infof(ctx, gjson.MustEncodeString(result))
	}
...
func TestReg(t *testing.T) {
	...
	for _, testCase := range testCases {
		logger.Info(ctx, gregex.IsMatch(openReg, []byte(testCase)))
	}
